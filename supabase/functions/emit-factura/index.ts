...
serve(async (req) => {
  ...
  try {
    ...
    console.log(`Processing factura emission for sale: ${saleId}`)
    ...
    console.log('🔍 [EMIT-FACTURA] Verifying assignments exist...')
      ...
      try {
        ...
        return new Response(
          JSON.stringify({
            ...
            serialNumber: venta.serial_number,
            detailsHref: venta.details_href,
            message: 'Factura completada exitosamente (reintento de consumo de stock)'
          }),
          { headers: { ...corsHeaders, 'Content-Type': 'application/json' }, status: 200 }
        )
      } catch (error) {
    ...
    const payload = buildFacturaPayload(venta, details, paymentTypeId, emissionDate, false)
    ...
    console.log('📦 [EMIT-FACTURA] Consuming stock from reserved...')
    try {
      await consumeStockFromReserved(supabase, saleId)
      console.log('✅ Stock consumed successfully from reserved')
    } catch (error) {
      console.error('❌ [EMIT-FACTURA] Stock consumption failed:', error)
    ...
    console.log('🗑️ [EMIT-FACTURA] Deleting assignments after successful consumption...')
