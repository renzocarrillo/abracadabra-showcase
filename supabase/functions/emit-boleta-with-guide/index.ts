...
async function createRemissionGuide(
  ...
  // Match with documentDetails to get netUnitValue
  const shippingDetails = ventaDetails.map(detail => {
    // Find matching document detail by SKU
  ...
  const payload = {
    ...
    priceListId: 31,
    shippingTypeId: 10,
    district: envioInfo.distrito || clienteInfo.distrito || 'Lima',
    city: envioInfo.provincia || clienteInfo.provincia || 'Lima',
    address: envioInfo.direccion || clienteInfo.direccion || '',
    declare: 1,
    recipient: clientRecipient,
    details: shippingDetails,
    client: {
      code: clienteInfo.ruc || clienteInfo.dni || '',
      district: envioInfo.distrito || clienteInfo.distrito || 'Lima',
...
serve(async (req) => {
  ...
  try {
    ...
    console.log(`Processing boleta with guide emission for sale: ${saleId}, transportist: ${transportistId || 'none'}`)
      ...
      if (venta.estado === 'archivado') {
        ...
        if (detailsNeedUpdate && venta.id_bsale_documento) {
          ...
          try {
            ...
            // Non-critical, continue returning existing data
          }
        }
        ...
        return new Response(
          JSON.stringify({
            ...
            message: 'Boleta con guía ya emitida anteriormente'
          }),
      ...
      try {
        ...
        return new Response(
          JSON.stringify({
            ...
            detailsHref: venta.details_href,
            urlGuidesPdf: venta.url_guia_remision || null,
            message: 'Boleta con guía completada exitosamente (reintento de consumo de stock)'
          }),
          { headers: { ...corsHeaders, 'Content-Type': 'application/json' }, status: 200 }
        )
      } catch (error) {
    ...
    console.log('🔍 [EMIT-BOLETA-WITH-GUIDE] Verifying assignments exist (new emission)...')
    ...
    const payload = buildBoletaPayload(venta, ventaDetails, details, paymentTypeId, emissionDate, true)
    ...
    if (transportistId) {
      console.log('🚚 [EMIT-BOLETA-WITH-GUIDE] Creating remission guide with transportist:', transportistId)
      try {
        ...
        // Continue with stock consumption and archiving
      }
    } else {
    ...
    console.log('📦 [EMIT-BOLETA-WITH-GUIDE] Consuming stock from reserved...')
    try {
      await consumeStockFromReserved(supabaseClient, saleId)
      console.log('✅ Stock consumed successfully from reserved')
    } catch (error) {
      console.error('❌ [EMIT-BOLETA-WITH-GUIDE] Stock consumption failed:', error)
    ...
    console.log('🗑️ [EMIT-BOLETA-WITH-GUIDE] Deleting assignments after successful consumption...')
    ...
    return new Response(
      JSON.stringify({
        ...
        message: guideUrl 
          ? 'Boleta con guía emitida exitosamente' 
          : 'Boleta emitida - Guía pendiente de emisión manual'
      }),
      { 
    ...
    console.error('Error emitting boleta with guide:', error)
    return new Response(
      JSON.stringify({
        success: false,
        error: error.message
