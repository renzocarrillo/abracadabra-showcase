...
import { makeBsaleRequest, buildTicketEmpresaPayload } from '../_shared/bsale-api.ts'
...
serve(async (req) => {
  ...
  try {
    ...
    console.log(`Processing ticket empresa emission for sale: ${saleId}`)
    ...
    console.log('🔍 [EMIT-TICKET-EMPRESA] Verifying assignments exist...')
    ...
    // 5. Build and send Bsale payload for ticket empresa
    const payload = buildTicketEmpresaPayload(venta, details, paymentTypeId, emissionDate)
    ...
    console.log('📦 [EMIT-TICKET-EMPRESA] Consuming stock from reserved...')
    try {
      await consumeStockFromReserved(supabase, saleId)
      console.log('✅ Stock consumed successfully from reserved')
    } catch (error) {
      console.error('❌ [EMIT-TICKET-EMPRESA] Stock consumption failed:', error)
    ...
    console.log('🗑️ [EMIT-TICKET-EMPRESA] Deleting assignments after successful consumption...')
    ...
    return new Response(
      JSON.stringify({
        ...
        message: 'Ticket empresa emitido exitosamente'
      }),
      { 
    ...
    console.error('Error emitting ticket empresa:', error)
    return new Response(
      JSON.stringify({
        success: false,
        error: error.message
