-- Documentación de la función scan_product_unified actual en la base de datos
-- Esta migración sirve para sincronizar el repositorio con la función que ya existe en producción

-- Primero eliminamos la versión anterior si existe
DROP FUNCTION IF EXISTS public.scan_product_unified(UUID, TEXT, TEXT);

-- Recreamos la función con la firma correcta: (p_session_id, p_scanned_code, p_user_id opcional)
...
BEGIN
  ...
  -- 5. Obtener el último bin escaneado en esta sesión
  SELECT bin_code INTO v_last_bin
...
La función obtiene el bin del último ítem escaneado en la sesión.';